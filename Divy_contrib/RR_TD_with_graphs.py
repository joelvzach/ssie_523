...
def find_country_key(country_name, base_data):
    wanted = normalize_country_name(country_name)
    possible = COUNTRY_ALIASES.get(wanted, [wanted])
    for name in possible:
        key = normalize_country_name(name)
        if key in base_data:
            return key
    # loose fallback: useful when the CSV has names like "United States of America"
    ...
    for file in DATA_FILES:
        ...
        if not os.path.exists(file):
            raise FileNotFoundError(
                ...
                "Make sure the file is inside D:\\RDTS, or update DATA_FILES at the top of this script."
            )
        df = pd.read_csv(file)
        print("Rows:", len(df))
...
def build_sim_countries():
    ...
    for country_name in SELECTED_COUNTRIES:
        key = find_country_key(country_name, base_data)
...
class Simulation:
    ...
    def reset(self):
        ...
        self.paused = False
        self.log = []
        self.history_total_staying = []
        self.history_overcrowded = []
        self.spawn_batch(16)
    ...
    def update(self):
        ...
        if self.tick % 10 == 0:
            total_staying = sum(c.visitors for c in self.countries)
            overcrowded_count = sum(1 for c in self.countries if c.overcrowded)
            self.history_total_staying.append(total_staying)
            self.history_overcrowded.append(overcrowded_count)
            self.history_total_staying = self.history_total_staying[-100:]
            self.history_overcrowded = self.history_overcrowded[-100:]
    ...
    def draw_bar_graph(self, x, y, w, h):
        title = self.font_tiny.render("Visitors vs capacity", True, TEXT_DIM)
        self.screen.blit(title, (x, y - 18))

        if not self.countries:
            return

        max_value = max(max(c.visitors, c.effective_capacity) for c in self.countries)
        max_value = max(max_value, 1)
        bar_gap = 6
        bar_w = max(10, (w - bar_gap * (len(self.countries) - 1)) // len(self.countries))
        base_y = y + h

        pygame.draw.line(self.screen, TEXT_DIM, (x, base_y), (x + w, base_y), 1)
        pygame.draw.line(self.screen, TEXT_DIM, (x, y), (x, base_y), 1)

        for i, c in enumerate(self.countries):
            bx = x + i * (bar_w + bar_gap)
            visitor_h = int((c.visitors / max_value) * h)
            cap_h = int((c.effective_capacity / max_value) * h)

            # capacity outline
            pygame.draw.rect(self.screen, TEXT_DIM, (bx, base_y - cap_h, bar_w, cap_h), 1)

            # visitor fill uses the same country status color
            pygame.draw.rect(self.screen, c.status_color(), (bx + 2, base_y - visitor_h, max(1, bar_w - 4), visitor_h))

            short_name = c.name[:3].upper()
            label = self.font_tiny.render(short_name, True, TEXT_DIM)
            self.screen.blit(label, (bx + bar_w // 2 - label.get_width() // 2, base_y + 4))

    def draw_line_graph(self, x, y, w, h):
        title = self.font_tiny.render("Tourists staying over time", True, TEXT_DIM)
        self.screen.blit(title, (x, y - 18))

        values = self.history_total_staying
        if len(values) < 2:
            msg = self.font_tiny.render("Graph starts after a few seconds", True, TEXT_DIM)
            self.screen.blit(msg, (x + 5, y + h // 2 - 8))
            return

        pygame.draw.line(self.screen, TEXT_DIM, (x, y + h), (x + w, y + h), 1)
        pygame.draw.line(self.screen, TEXT_DIM, (x, y), (x, y + h), 1)
        ...
        for i, value in enumerate(values):
            px = x + int(i * step)
            py = y + h - int((value / max_v) * h)
            points.append((px, py))

        if len(points) >= 2:
            pygame.draw.lines(self.screen, YELLOW, False, points, 2)

        latest = self.font_tiny.render(str(values[-1]), True, TEXT_LIGHT)
        self.screen.blit(latest, (x + w - latest.get_width(), y - 2))
    ...
    def draw_panel(self):
        ...
        y += 10
        self.draw_bar_graph(px + 12, y + 18, pw - 24, 90)
        y += 132
        self.draw_line_graph(px + 12, y + 18, pw - 24, 80)
        y += 118
